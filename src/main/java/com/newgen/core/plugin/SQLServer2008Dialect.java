...
public class SQLServer2008Dialect {
	...
	public String getAppPageString(String sql ,int startid,int count){
		
		StringBuilder sb = new StringBuilder(sql.trim().toLowerCase());
		
		// select * from t_news where 1=1 and catligf=4 order by   f.topflag DESC,f.topTime DESC
		...
		selectPageSql(sb,startid,count);
 ...
 * @param count
 */
	private void selectPageSql(StringBuilder sb, int startid, int count) {
		
		
		String startsql = sb.toString();
		
		
		StringBuilder builderbetweeafter = new StringBuilder(startsql);
		StringBuilder builderandafter = new StringBuilder(startsql);
		
		sb.insert(0, "select * from (").append(") f");
		
		
		StringBuilder betweeafter = builderbetweeafter.insert(0, "(select b.__liud_row_nr__ from (").append(") b where b.id =").append(startid).append(")").append("+1");
		StringBuilder andafter = builderandafter.insert(0, "(select b.__liud_row_nr__ from (").append(") b  where b.id =").append(startid).append(")").append("+").append(count);
		...
		if(startid==-1){
			...
			sb.append(" where f.__liud_row_nr__ BETWEEN ").append(1)
			.append(" and ").append(count);			
			
		}
		else {
			
			sb.append(" where f.__liud_row_nr__ BETWEEN ").append(betweeafter)
			.append(" and ").append(andafter);	
