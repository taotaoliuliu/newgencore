...
public class Base64 {
    ...
    static void compare(char[] b1, char[] b2) {
        if (b1 == null || b2 == null) {
            System.out.println("Base64.compare(char) >> Null array!");
        } else if (b1.length != b2.length) {
            System.out.println("Base64.compare(char) >> arrays are different lengths!");
        } else for (int i=0; i<b1.length; i++) {
            if (b1[i] != b2[i]) {
                System.out.println("Base64.compare(char) >> arrays disagree at byte " + i);
                return;
            }
        }
    }
    ...
    static void compare(byte[] b1, byte[] b2) {
        if (b1 == null || b2 == null) {
            System.out.println("Base64.compare(byte) >> Null array!");
            return;
        }
        if (b1.length != b2.length) {
            System.out.println("Base64.compare(byte) >> arrays are different lengths!");
            return;
        }
        for (int i=0; i<b1.length; i++) {
            if (b1[i] != b2[i]) {
                System.out.println("Base64.compare(byte) >> arrays disagree at byte " + i);
                return;
            }
        }
    }
